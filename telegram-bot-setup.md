...
| После перезапуска бота пропали устройства из `/mydevices` | это `state.json` — проверьте, что он не был случайно удалён; сами устройства в `vpn.env` при этом не пострадали, их видно через `/devices` (строка «добавлено не через бота») |
