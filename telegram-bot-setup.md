...
Идите по этапам сверху вниз: каждый опирается на предыдущие.

1. **Этап 1. Домены и DNS** — панель DNS и ваш компьютер. Начните с него: пока записи расходятся по интернету, можно настраивать серверы.
2. **Этап 2. Базовая настройка серверов** — оба сервера: firewall, SSH, fail2ban, пакеты и Docker.
3. **Этап 3. Файл значений и ключи** — RU-сервер, затем копия на EU.
4. **Этап 4. Европейский сервер** — EU.
5. **Этап 5. Российский сервер** — RU.
6. **Этап 6. Проверка** — ваш компьютер и RU.
7. **Этап 7. Подключение устройств** — RU и устройства.
8. **Этап 8. Обслуживание** — по мере необходимости.
9. **Этап 9. Telegram-бот для самообслуживания** — RU, по желанию: чтобы семья сама добавляла себе устройства в общем чате.

В конце инструкции — «Справочник»: команды `vpnctl.sh`, текст скрипта и ручные способы сгенерировать ключи и собрать ссылки.

## Этап 1. Домены и DNS — панель DNS и ваш компьютер
...
### 1.1. Узнайте, где управляется DNS
...
### 1.2. Записи для поддоменов клининга

1. Войдите в панель, найденную в шаге 1.1, откройте домен `CLEANING_DOMAIN` и раздел «DNS-записи», «Управление зоной» или «DNS-редактор».
...
### 1.3. Новый домен для EU-сервера
...
### 1.5. Убедитесь, что записи работают
...
## Этап 2. Базовая настройка серверов — оба сервера

Выполняется на обоих серверах: сначала закрываем доступ, потом ставим пакеты. Файла значений ещё нет, поэтому `SSH_PORT` и `IP_СЕРВЕРА` на этом этапе вписывайте руками. В итоге наружу будут открыты только SSH на нестандартном порту, 80 и 443, а вход — по паролю с защитой от подбора.

### 2.1. Firewall — оба сервера
...
ufw allow 22/tcp          # временно, закроем в 2.2
...
Вход остаётся по паролю, поэтому пароль должен быть длинным и случайным. Сгенерируйте его и сохраните в менеджере паролей, затем установите для root:

```bash
openssl rand -base64 18      # случайный пароль из 24 символов
passwd                       # вставить его дважды
```

Создайте `/etc/ssh/sshd_config.d/10-hardening.conf`. Номер `10` в имени важен: из файлов в этой папке побеждает прочитанный первым, а в облачных образах Ubuntu бывает `50-cloud-init.conf`, который может запрещать пароли. Вместо `SSH_PORT` впишите выбранный порт, например `2222`.
...
### 2.3. fail2ban — оба сервера

При входе по паролю fail2ban — главная защита от подбора, поэтому правила строже обычного: после 3 неудачных попыток за 10 минут IP блокируется на сутки. По умолчанию fail2ban следит за портом 22, а в Debian 12 без `backend = systemd` вообще не стартует. Файл `/etc/fail2ban/jail.local`, вместо `SSH_PORT` впишите свой порт:
...
### 2.4. Автообновления безопасности — оба сервера

```bash
dpkg-reconfigure -plow unattended-upgrades   # ответить «Да»
```

### 2.5. Пакеты и Docker — оба сервера

Первый блок одинаков для обоих серверов, дальше — отдельно для EU и RU.

```bash
# оба сервера
apt update && apt upgrade -y
apt install -y certbot openssl curl jq qrencode
curl -fsSL https://get.docker.com | sh
mkdir -p /var/www/stub /var/www/letsencrypt
...
# RU-сервер (nginx уже стоит, на нём работает сайт)
mkdir -p /opt/ru-proxy
nginx -V 2>&1 | grep -o 'with-stream[=a-z]*'   # встроен ли модуль stream
ls /etc/nginx/modules-enabled/ | grep stream   # подключён ли он отдельным модулем
# «with-stream» без «=dynamic» — модуль встроен, ставить ничего не нужно.
# «with-stream=dynamic», а вторая команда ничего не вывела (обычно так в Ubuntu):
apt install -y libnginx-mod-stream
```

## Этап 3. Файл значений и ключи — RU, затем EU

Все значения схемы хранятся в одном файле `/root/vpn.env`. Скрипт `vpnctl.sh` генерирует ключи и секреты, подставляет значения в конфиги, ведёт список устройств и собирает ссылки. Всё создаём на RU-сервере (там же потом добавляются устройства), а затем копируем на EU.

### 3.1. Установка скрипта — ваш компьютер и RU

Скрипт приложен к инструкции файлом `vpnctl.sh`, его полный текст есть и в «Справочнике». Загрузите файл на RU-сервер с компьютера. Если удобнее, откройте на сервере `nano /usr/local/bin/vpnctl.sh` и вставьте текст из «Справочника».

```bash
# ваш компьютер (в папке со скачанным vpnctl.sh)
scp -P SSH_PORT vpnctl.sh root@RU_IP:/usr/local/bin/vpnctl.sh
...
### 3.2. Файл значений и генерация ключей — RU

Создайте `/root/vpn.env` и заполните первый блок: IP, домены из этапа 1, порт SSH. Остальное оставьте пустым.

Домены — это имена, для которых вы создали A-записи на этапе 1:

| Переменная | Что вписать | Пример |
| --- | --- | --- |
| `CLEANING_DOMAIN` | домен сайта клининга | `cleaning-spb.ru` |
| `TG_HOST` | поддомен клининга для Telegram-прокси | `static.cleaning-spb.ru` |
| `VPN_HOST` | поддомен клининга для основного VPN-профиля | `media.cleaning-spb.ru` |
| `VPN2_HOST` | поддомен клининга для запасного профиля | `api.cleaning-spb.ru` |
| `EU_HOST` | купленный домен для EU-сервера целиком | `northstudio.fun` |

Вписывайте имена без `https://` и без точки в конце. Если в шаге 1.2 вы взяли другие поддомены (`cdn`, `img`, `files`), впишите их.
...
vpnctl.sh show        # проверить, что получилось (приватные ключи — частично)
```

`init` заполняет только пустые значения, поэтому его безопасно запускать повторно. Чтобы перевыпустить что-то одно, очистите это значение в файле и запустите `init` снова.
...
### 3.3. Что хранится в vpn.env

После `vpnctl.sh init` в `vpn.env` лежат все значения схемы. В конфигах и командах инструкции они записаны плейсхолдерами `КАПСОМ`:
...
| `MTG_SECRET` | секрет Telegram-прокси | vpnctl.sh init |
...
### 3.5. Значения как переменные в терминале — оба сервера
...
### 3.6. Копия на EU-сервер — ваш компьютер и EU

```bash
# ваш компьютер (в той же папке, где vpnctl.sh)
scp -P SSH_PORT root@RU_IP:/root/vpn.env .
scp -P SSH_PORT vpn.env vpnctl.sh root@EU_IP:/root/
rm vpn.env
...
Файл копируется через ваш компьютер, потому что серверы не умеют заходить друг на друга по SSH. Главная копия `vpn.env` — на RU-сервере, правьте её там. Если поменяли что-то, что нужно EU-серверу (ключи EU, `EU_PATH`, `EU_HOST`), скопируйте файл на EU тем же способом.

### 3.7. Как подставлять значения в конфиги

Конфиги из инструкции вставляйте как есть, с плейсхолдерами, а потом подставляйте значения командой `vpnctl.sh render ФАЙЛ`. Скрипт оставляет копию с расширением `.bak` и показывает плейсхолдеры, которые заменить не удалось. В командах вместо плейсхолдеров используются переменные (`"$EU_HOST"`). На этапах 4, 5 и 8 нужные вызовы `render` уже вписаны в блоки кода, отдельно ничего делать не нужно.

## Этап 4. Европейский сервер — EU

На EU-сервере Xray сам слушает порт 443 и принимает VLESS+XHTTP+Reality. Всем, кто не знает ключа, он показывает сайт-заглушку с локального nginx. Конфиги вставляйте как есть: команды `vpnctl.sh render` в блоках ниже подставят значения из `vpn.env`.

### 4.1. Сайт-заглушка и сертификат — EU
...
```bash
vpnctl.sh render /etc/nginx/sites-available/stub /var/www/stub/index.html
...
```bash
vpnctl.sh render /etc/nginx/sites-available/stub
nginx -t && systemctl reload nginx
```
...
## Этап 5. Российский сервер — RU

На RU-сервере порт 443 займёт nginx в режиме `stream` и будет раздавать подключения по имени домена. Сайт клининга и три заглушки переедут на `127.0.0.1:8443`. Порядок шагов выбран так, чтобы сайт был недоступен не дольше пары секунд при перезапуске nginx. Конфиги вставляйте как есть: значения подставят команды `vpnctl.sh render` в блоках ниже.

### 5.1. Заглушки и сертификаты для масок — RU

Страница-заглушка: такая же, как в шаге 4.1, но с другим названием и текстом, чтобы сайты на двух серверах не выглядели одинаково. Положите её в `/var/www/stub/index.html`.
...
```bash
vpnctl.sh render /etc/nginx/sites-available/masks
...
### 5.2. Подготовка переключения — RU
...
В основном server-блоке сайта (с `server_name CLEANING_DOMAIN`) строка `listen` должна быть одна и заканчиваться на `default_server`. Тогда на неизвестные имена nginx будет отдавать сайт клининга, а не одну из заглушек:
...
```bash
vpnctl.sh render /etc/nginx/stream.conf /etc/nginx/sites-available/masks
nginx -t && systemctl restart nginx
curl -I "https://$CLEANING_DOMAIN"           # сайт должен ответить 200 или 301
ss -tlnp | grep -E ':(80|443|8443)\b'
```

Ожидаемо: `0.0.0.0:443` и `0.0.0.0:80` — nginx, `127.0.0.1:8443` — nginx. Если `nginx -t` ругается на занятый адрес, значит в `http {}` осталась строка с 443 (шаг 5.2). Если ошибка `unknown directive "stream"`, не подключён модуль stream: выполните `apt install -y libnginx-mod-stream` (шаг 2.5). Прокси пока не работают: их поднимем в шагах 5.4–5.7.
...
```json
{
  ...
  "routing": {
    ...
    "rules": [
      { "type": "field", "inboundTag": ["dns-internal"], "outboundTag": "to-eu" },
      { "type": "field", "inboundTag": ["tg-socks"], "outboundTag": "to-eu" },
      { "type": "field", "ip": ["geoip:private"], "outboundTag": "block" },
      { "type": "field", "protocol": ["bittorrent"], "outboundTag": "block" },
      { "type": "field", "domain": ["geosite:category-ru", "geosite:avito"], "outboundTag": "direct" },
...
- Российские сайты (`geosite:category-ru`) и российские IP (`geoip:ru`) выходят в интернет прямо с RU-сервера, минуя Европу. Что входит в эти списки и как добавить исключения — в шаге 5.8.
- `"domainStrategy": "IPIfNonMatch"` в `routing`: если домен не попал ни в один список, Xray узнаёт его IP и проверяет по `geoip:ru`. Свои DNS-запросы Xray отправляет в Cloudflare (`1.1.1.1`) через туннель в Европу (первое правило), поэтому провайдер не может подменить ответ для заблокированного сайта. Прямой выход (`direct`) резолвит домены обычным DNS сервера (`AsIs`), чтобы российские сервисы отдавали свои российские адреса.
...
Секрет `MTG_SECRET` уже сгенерирован на этапе 3 командой `vpnctl.sh init`. Значения в этот файл подставятся в шаге 5.6.
...
### 5.6. Подстановка значений и устройства — RU

Подставьте значения в оба конфига и запишите в `xray.json` список устройств. Имена устройств пишите латиницей: они попадут в название подключения в приложении.
...
После `render` скрипт сообщит, что в `xray.json` остались `UUID_PHONE` и `UUID_LAPTOP`. Это нормально: их заменяет список устройств командой `sync-clients`.
...
Списки российских сайтов (`geosite.dat`, `geoip.dat`) внутри образа Xray заморожены на дату его сборки, а новые домены в них добавляются постоянно. Поэтому свежие списки лежат отдельно, в папке `/opt/ru-proxy/geo`, и раз в неделю обновляются сами. Скрипт сначала проверяет конфиг с новыми списками и только потом подменяет старые:

```bash
cat > /usr/local/bin/update-geo.sh <<'EOF'
#!/bin/sh
...
EOF
chmod +x /usr/local/bin/update-geo.sh

update-geo.sh                                                    # первая загрузка
ls -la /opt/ru-proxy/geo      # должны быть geoip.dat и geosite.dat по 10–17 МБ
echo '30 4 * * 1 root /usr/local/bin/update-geo.sh' > /etc/cron.d/update-geo   # по понедельникам в 4:30
```

Не переходите к запуску, пока в `/opt/ru-proxy/geo` не лежат оба файла. Если папка пустая, Docker подключит её вместо встроенных списков и Xray не запустится. Если скрипт пишет «не удалось скачать», значит RU-сервер не достучался до GitHub напрямую: временно закомментируйте строку `- ./geo:/usr/share/xray:ro` в `docker-compose.yml`, запустите сервисы, выполните `update-geo.sh` ещё раз (теперь скачивание пойдёт через туннель), затем верните строку и выполните `docker compose up -d`.
...
### 5.8. Что идёт напрямую, а что через Европу — RU
...
| Порядок | Трафик | Куда |
| --- | --- | --- |
| 1 | DNS-запросы самого Xray | Европа (к Cloudflare) |
| 2 | Telegram-прокси (mtg) | Европа |
| 3 | локальные адреса (`geoip:private`) | блокируется |
| 4 | торренты | блокируется |
| 5 | российские домены (`geosite:category-ru`, `geosite:avito`) | напрямую с RU-сервера |
| 6 | российские IP (`geoip:ru`), в том числе у доменов не из списков | напрямую с RU-сервера |
| 7 | всё остальное | Европа |

`geosite:category-ru` — список из базы v2fly: все российские доменные зоны (`.ru`, `.su`, `.рф`, `.moscow`, `.tatar` и другие), а также VK, Яндекс, Mail.ru, банки, маркетплейсы и операторы связи на доменах вроде `.com`. Если у российского сервиса окажется домен, которого нет в списках, он всё равно пойдёт напрямую, когда его сервер стоит в России: Xray определяет IP такого домена и проверяет его по `geoip:ru` (см. 5.4). Сами списки обновляются автоматически раз в неделю (шаг 5.7).
...
**Если RU-сервер уже настроен по более ранней версии инструкции**, обновите маршрутизацию одной командой. Её можно запускать повторно: дубликатов она не создаёт, а ваши исключения сохраняет.

```bash
# RU-сервер
cd /opt/ru-proxy
jq '.dns = {"servers": ["https://1.1.1.1/dns-query"], "queryStrategy": "UseIPv4", "tag": "dns-internal"}
  | .routing.domainStrategy = "IPIfNonMatch"
  | .routing.rules |= (if any(.[]; .inboundTag == ["dns-internal"]) then . else [{"type": "field", "inboundTag": ["dns-internal"], "outboundTag": "to-eu"}] + . end)
  | (.routing.rules[] | select(.outboundTag == "direct" and has("domain")) | .domain) |= ((. - ["domain:ru", "domain:su", "domain:xn--p1ai"]) + ["geosite:category-ru", "geosite:avito"] | unique)
  | (.outbounds[] | select(.tag == "direct") | .settings.domainStrategy) = "AsIs"' \
  xray.json > /tmp/xray.json && cp /tmp/xray.json xray.json
docker run --rm -v /opt/ru-proxy/xray.json:/etc/xray/config.json:ro \
  teddysun/xray:26.3.27 xray -test -config /etc/xray/config.json
```

Затем включите свежие списки: создайте `update-geo.sh` и cron из шага 5.7 и запустите `update-geo.sh`. Пока Xray работает, скачивание идёт через туннель. Убедитесь, что `ls -la /opt/ru-proxy/geo` показывает два файла по 10–17 МБ. Только после этого добавьте в `docker-compose.yml` у сервиса `xray` строку тома `- ./geo:/usr/share/xray:ro` и пересоздайте контейнер командой `docker compose up -d`. С пустой папкой Xray не запустится, и перестанут работать и VPN, и Telegram-прокси.

Команды из этого шага я проверил на конфигах из разных версий инструкции с Xray 26.3.27 и актуальными geo-файлами: `Configuration OK`, повторный запуск ничего не меняет.

## Этап 6. Проверка — ваш компьютер и RU
...
docker run --rm --network host \
  ...
  ghcr.io/9seconds/mtg:2 doctor --skip-native-check /config/config.toml
```

Флаг `--skip-native-check` пропускает попытку подключиться к Telegram напрямую: из России она всегда заканчивается таймаутом, а mtg так и не ходит. В выводе `doctor` должны быть зелёные отметки в трёх разделах: связь через `socks5://127.0.0.1:1080`, доступность заглушки (fronting domain) и совпадение `TG_HOST` с `RU_IP` (SNI-DNS match).
...
### 7.1. Получить ссылки — RU

Скрипт соберёт ссылки для всех устройств из шага 5.6, ссылку на Telegram-прокси и аварийную ссылку.

```bash
vpnctl.sh links                  # ссылки для всех устройств
...
### 7.3. Подключение VPN — устройства
...
### 7.4. Telegram-прокси — устройства
...
## Этап 8. Обслуживание и действия при блокировках
...
### Добавить или отключить устройство — RU

```bash
# новое устройство
vpnctl.sh add-device papa-laptop
vpnctl.sh sync-clients
cd /opt/ru-proxy && docker compose restart xray
vpnctl.sh links papa-laptop

# отключить устройство
vpnctl.sh remove-device papa-laptop
vpnctl.sh sync-clients
cd /opt/ru-proxy && docker compose restart xray
```
...
Списки российских сайтов обновляются сами раз в неделю (шаг 5.7). Результаты: `journalctl -t update-geo`. Обновить вручную: `update-geo.sh`.
...
EOF
vpnctl.sh render /usr/local/bin/tunnel-check.sh
...
| Российский сайт с VPN не открывается или открывается криво | часть его доменов уходит в Европу, или сайт не пускает IP дата-центра | с RU-сервера: `curl -sI https://сайт`. Ответ 200/301 — добавить его домены в прямой выход (исключения в шаге 5.8); 403/429 — открывать этот сайт без VPN |
...
На новом сервере повторите этап 4, используя те же ключи, UUID и путь. Переведите A-запись `EU_HOST` на новый IP и получите сертификат заново. На RU-сервере в `xray.json` поменяйте только `address` в outbound `to-eu` и выполните `docker compose restart xray`. Устройствам нужно обновить только аварийный профиль.
...
## Этап 9. Telegram-бот для самообслуживания — RU

После того как основной VPN и Telegram-прокси уже работают (этапы 1–8), можно добавить бота: он позволяет каждому в семейном чате самому добавить себе устройство, получить ссылку или QR-код — без того, чтобы каждый раз просить вас заходить на сервер по SSH.

Бот работает **только на RU-сервере**, использует уже настроенный `vpnctl.sh` и файл `/root/vpn.env` — вся логика генерации ключей и ссылок остаётся там, бот её не дублирует. Файлы бота (`bot.py`, `requirements.txt`, `vpnbot-helper.sh`, `vpnbot.service`, `vpnbot.sudoers`, `.env.example`) лежат в папке `vpn-bot/` рядом с этой инструкцией.

### Что умеет бот

| Команда | Что делает | Кто может |
| --- | --- | --- |
| `/add имя` | добавить новое устройство, получить ссылки и QR | все в семейном чате |
| `/claim имя` | привязать к себе устройство, выпущенное до появления бота | все в семейном чате |
| `/mydevices` | список своих устройств с кнопками | все |
| `/links [имя]` | прислать ссылки ещё раз | владелец устройства, админ |
| `/qr [имя]` | прислать QR-коды ещё раз | владелец устройства, админ |
| `/remove [имя]` | удалить устройство (с подтверждением) | владелец устройства, админ |
| `/telegram` | ссылка на Telegram-прокси | все |
| `/emergency` | аварийный профиль (см. этап 7) | все |
| `/devices` | список всех устройств и их владельцев | только админ |
| `/status` | состояние контейнеров xray/mtg и туннеля | только админ |
| `/whoami` | ваш Telegram ID и ID текущего чата | все — нужно для настройки |

Добавление и удаление устройства сами перезапускают Xray на RU-сервере — заходить на сервер руками не нужно.

### Как это устроено

```mermaid
flowchart LR
  U[Член семьи<br/>в семейном чате] -->|/add mama-phone| B[vpnbot<br/>пользователь vpnbot, без root]
  B -->|sudo -n, одна команда| H[vpnbot-helper.sh<br/>root]
  H --> V[vpnctl.sh]
  V --> E[/root/vpn.env/]
  H --> D[docker compose restart xray]
  B -->|ссылки и QR в личку| U
```

Три принципа безопасности, на которых это построено:

- **Бот не работает от root.** У него нет прямого доступа к `/root/vpn.env`. Всё, что требует привилегий (изменение `vpn.env`, перезапуск Xray), идёт через `sudo -n` и ровно один скрипт — `vpnbot-helper.sh`. В sudoers для пользователя `vpnbot` разрешён только этот один скрипт.
- **Секреты уходят только в личные сообщения.** В самом семейном чате остаётся только «Отправил вам в личку ✅» — ссылки вида `vless://...` и QR-коды с ключами в истории группового чата не оседают. Если человек ещё не открывал чат с ботом один на один, бот попросит нажать Start в личке и повторить команду.
- **Бот отвечает только в семейном чате и в личке с уже знакомыми людьми.** Если кто-то добавит бота в другой чат или напишет ему в личку, ни разу не засветившись в семейном чате, бот промолчит.

Отдельно: `/add` ограничен `MAX_DEVICES_PER_USER` (по умолчанию 5) на человека, чтобы никто по ошибке или в шутку не наплодил кучу устройств. Порт бот не открывает: он ходит к Telegram сам, в исходящем соединении (long polling), поэтому ничего в ufw и nginx менять не нужно.

### 9.1. Создать бота — ваш телефон или компьютер

1. Откройте Telegram, найдите **@BotFather**.
2. `/newbot`, придумайте имя и username (должен заканчиваться на `bot`, например `family_vpn_helper_bot`).
3. BotFather пришлёт токен вида `123456789:AAExampleTokenDoNotUseThisOne`. Это секрет уровня пароля от `vpn.env` — храните так же аккуратно, не выкладывайте никуда.
4. По желанию: `/setdescription` и `/setuserpic`, чтобы бот в семейном чате выглядел понятнее.

### 9.2. Установка на RU-сервере — RU

#### 9.2.1. Копирование файлов — ваш компьютер и RU

```bash
# ваш компьютер (в папке VPN, где лежит vpn-bot/)
scp -P SSH_PORT -r vpn-bot root@RU_IP:/opt/vpn-bot
```

#### 9.2.2. Системный пользователь и Python-окружение — RU

Бот работает не от root, а от отдельного системного пользователя без прав входа:

```bash
useradd --system --home /opt/vpn-bot --shell /usr/sbin/nologin vpnbot
...
#### 9.2.3. Helper-скрипт и sudoers — RU

`vpnbot-helper.sh` — единственная команда, которую боту разрешено выполнять от root. Он сам проверяет имя устройства и вызывает только `vpnctl.sh` и `docker compose restart xray`, ничего больше.

```bash
install -m 755 -o root -g root /opt/vpn-bot/vpnbot-helper.sh /usr/local/bin/vpnbot-helper.sh

install -m 440 -o root -g root /opt/vpn-bot/vpnbot.sudoers /etc/sudoers.d/vpnbot
visudo -c        # проверка синтаксиса всех файлов sudoers, должно быть "OK"
```

Проверка, что sudoers настроен правильно:

```bash
sudo -u vpnbot sudo -n /usr/local/bin/vpnbot-helper.sh status
```

Если команда просит пароль или отвечает `sudo: a password is required`, значит `/etc/sudoers.d/vpnbot` не подхватился — проверьте права (`440`, владелец `root:root`) и что `visudo -c` не ругается.

#### 9.2.4. Права на папки — RU

```bash
mkdir -p /opt/vpn-bot/links
chown -R vpnbot:vpnbot /opt/vpn-bot
chmod 700 /opt/vpn-bot/links
```

Файл со ссылками и state.json бот создаст сам при первом запуске.

### 9.3. Настройка — RU

#### 9.3.1. Узнать ID чата и свой ID

Настройка бота — курица и яйцо: чтобы бот работал только в семейном чате, нужно знать его ID, а чтобы узнать ID, проще всего один раз ненадолго запустить бота вообще без ограничений.

Сначала проверьте, достаёт ли RU-сервер сам до Telegram — часто не достаёт, и тогда без прокси бот падает с `httpx.ConnectTimeout`, даже не дойдя до `/whoami`:

```bash
curl -sS -o /dev/null -w "напрямую: %{http_code}\n" --max-time 8 https://api.telegram.org || echo "напрямую: не отвечает"
curl -sS -o /dev/null -w "через туннель: %{http_code}\n" --max-time 8 -x socks5h://127.0.0.1:1080 https://api.telegram.org || echo "через туннель: не отвечает"
```

Если первая команда не ответила (таймаут, `000`), а вторая вернула код (`200`, `401` — не важно, главное не таймаут), добавляйте `TELEGRAM_PROXY` сразу:

```bash
# RU-сервер, от имени vpnbot, временно без .env — только токен
sudo -u vpnbot BOT_TOKEN="ВАШ_ТОКЕН" TELEGRAM_PROXY="socks5://127.0.0.1:1080" \
  /opt/vpn-bot/venv/bin/python3 /opt/vpn-bot/bot.py
```

Если же первая команда ответила сама — прокси не нужен, запускайте без `TELEGRAM_PROXY`:

```bash
sudo -u vpnbot BOT_TOKEN="ВАШ_ТОКЕН" /opt/vpn-bot/venv/bin/python3 /opt/vpn-bot/bot.py
```

Дальше в Telegram:

1. Добавьте бота в семейный чат (обычные права участника, без админства).
2. Напишите в чате `/whoami` — бот ответит ID чата (число с минусом).
3. Напишите боту `/whoami` в личных сообщениях — бот ответит ваш личный ID.

Остановите тестовый запуск: `Ctrl+C`.

#### 9.3.2. Файл настроек — RU
...
| `FAMILY_CHAT_ID` | ID чата из шага 9.3.1, с минусом |
| `ADMIN_IDS` | ваш личный ID из шага 9.3.1; через запятую можно добавить ещё администраторов |
| `MAX_DEVICES_PER_USER` | оставить `5`, если не нужно другое значение |
| `TELEGRAM_PROXY` | по результату проверки в шаге 9.3.1: пусто, если сервер сам достучался, иначе `socks5://127.0.0.1:1080` |

```bash
chown vpnbot:vpnbot /opt/vpn-bot/.env
...
```bash
install -m 644 /opt/vpn-bot/vpnbot.service /etc/systemd/system/vpnbot.service
systemctl daemon-reload
systemctl enable --now vpnbot
systemctl status vpnbot        # должно быть "active (running)"
journalctl -u vpnbot -f        # логи в реальном времени, Ctrl+C чтобы выйти
```

#### 9.4.2. Проверка

В семейном чате: `/add test-device`. Бот должен отчитаться о добавлении и написать вам в личку ссылки и два QR-кода. Проверьте, что устройство подключается (этап 7), затем удалите тестовое: `/remove test-device`.

#### 9.4.3. Если бот не подключается к Telegram

Если пропустили проверку в шаге 9.3.1 и в логе (`journalctl -u vpnbot`) видно `httpx.ConnectTimeout` или `TimedOut` при старте — RU-сервер не достучался до `api.telegram.org` напрямую. Заведите боту тот же путь в Европу, что уже использует Telegram-прокси:
...
Это тот же SOCKS-порт, что слушает Xray для `mtg` (шаг 5.4), отдельно ничего настраивать не нужно.

### Как пользоваться ботом — для всей семьи

1. Откройте семейный чат, где есть бот.
2. `/add имя-устройства` — имя латиницей, например `mama-phone` или `papa-laptop`. Бот подождёт несколько секунд (перезапускает сервер) и пришлёт вам в личные сообщения основную и запасную ссылку плюс два QR-кода.
3. Если это первое сообщение боту — прежде чем он сможет написать в личку, откройте с ним чат один на один и нажмите **Start**, затем повторите `/add` в семейном чате.
4. Дальше действуйте по [инструкции для подключения устройств](instrukciya-dlya-roditeley.md) — она про то, что делать с уже полученной ссылкой.
5. Понадобится ссылка ещё раз (например, на новом телефоне переустановили приложение) — `/links имя-устройства` или `/qr имя-устройства`. Если устройство у вас одно, можно без имени: просто `/links`.
6. Устройство больше не нужно — `/remove имя-устройства`, бот переспросит перед удалением.
7. Ссылка на Telegram-прокси — `/telegram`, она одна общая для всех.
...
```bash
scp -P SSH_PORT -r vpn-bot root@RU_IP:/opt/vpn-bot   # с вашего компьютера
cd /opt/vpn-bot && ./venv/bin/pip install -r requirements.txt
systemctl restart vpnbot
```

#### Посмотреть, кто что добавил — RU

```bash
cat /opt/vpn-bot/state.json | jq .
```

Или прямо в чате: `/devices` (только для администраторов).

#### Второй администратор

Добавьте его Telegram ID через запятую в `ADMIN_IDS` в `/opt/vpn-bot/.env`, затем `systemctl restart vpnbot`.

#### Если токен утёк

Отзовите его у BotFather (`/revoke` в диалоге с BotFather для этого бота), получите новый, впишите в `.env`, перезапустите сервис. Старый токен сразу перестаёт работать.

#### Резервная копия

`/opt/vpn-bot/.env` и `/opt/vpn-bot/state.json` стоит сохранять туда же, куда `vpn.env` — в зашифрованный архив (см. конец этапа 8). Без них при переустановке бот просто забудет, кто чем владеет; сами VPN-доступы при этом не пострадают, потому что они хранятся в `vpn.env`, а не в боте.

### Диагностика бота

| Симптом | Что проверить |
| --- | --- |
| `systemctl status vpnbot` — `failed` | `journalctl -u vpnbot -n 50` — обычно опечатка в `.env` или не установлены зависимости |
| Бот молчит в семейном чате | `FAMILY_CHAT_ID` в `.env` совпадает с ID из `/whoami`? Перезапущен ли сервис после правки `.env`? |
| «Не получилось добавить устройство» | `sudo -u vpnbot sudo -n /usr/local/bin/vpnbot-helper.sh add test123` вручную — покажет настоящую ошибку от `vpnctl.sh` или `docker` |
| Бот не пишет ссылки в личку | получатель не нажал Start в чате с ботом — бот сам подскажет это в семейном чате |
| `/status` и `/devices` не отвечают | ваш ID не попал в `ADMIN_IDS`, проверьте `/whoami` |
| После перезапуска бота пропали устройства из `/mydevices` | это `state.json` — проверьте, что он не был случайно удалён; сами устройства в `vpn.env` при этом не пострадали, их видно через `/devices` (строка «добавлено не через бота»), а владелец возвращает их себе командой `/claim имя` |
| Устройства, выпущенные до появления бота, не видны в `/mydevices` | это ожидаемо — бот узнаёт о них только из `state.json`, а не из `vpn.env`. Владелец привязывает такое устройство к себе командой `/claim имя` (UUID и ссылки не меняются, переподключать устройство не нужно); что привязывать — видно в `/devices` |

## Справочник

Справочные материалы: команды скрипта, его полный текст и ручные способы, если скрипт по какой-то причине не подходит.
...
| `vpnctl.sh show` | показывает значения (приватные ключи — первыми 4 символами) | оба |
...
### Текст скрипта vpnctl.sh

Этот же текст приложен отдельным файлом `vpnctl.sh`. Установка — шаг 3.1.
...
# Остановиться, если ключи не удалось разобрать из вывода xray x25519
check_pair() {
  [ -n "$1" ] && [ -n "$2" ] || die "не удалось получить ключи из xray x25519. Пришлите вывод команды: docker run --rm $XRAY_IMAGE xray x25519"
}
...
cmd_init() {
  ...
  if [ -z "${RU_PRIVATE_KEY:-}" ]; then
    read -r a b <<<"$(gen_pair)"; check_pair "$a" "$b"; set_var RU_PRIVATE_KEY "$a"; set_var RU_PUBLIC_KEY "$b"
  fi
  if [ -z "${EU_PRIVATE_KEY:-}" ]; then
    read -r a b <<<"$(gen_pair)"; check_pair "$a" "$b"; set_var EU_PRIVATE_KEY "$a"; set_var EU_PUBLIC_KEY "$b"
  fi
...
cmd_show() {
  load
  # приватные ключи показываются только первыми 4 символами
  sed -E 's/^([A-Z_]*_PRIVATE_KEY)="(.{4})[^"]*"/\1="\2…"/' "$ENV_FILE"
  echo "(приватные ключи показаны частично; целиком: grep PRIVATE $ENV_FILE)"
}
...
Использование: vpnctl.sh КОМАНДА [аргументы]
  ...
  show                     показать значения (приватные ключи — частично)
...
### Ключи и секреты вручную

Если вы не пользуетесь `vpnctl.sh`, сгенерируйте значения вручную на любом сервере с Docker и впишите их в `vpn.env` или сразу в конфиги вместо плейсхолдеров.
...
Секрет Telegram-прокси (`MTG_SECRET`):
...
### Ссылка на Telegram-прокси вручную
