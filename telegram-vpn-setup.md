...
# если предыдущая команда ничего не вывела:
...
Ожидаемо: `0.0.0.0:443` и `0.0.0.0:80` — nginx, `127.0.0.1:8443` — nginx. Если `nginx -t` ругается на занятый адрес, значит в `http {}` осталась строка с 443 (шаг 5.2). Прокси пока не работают: их поднимем в шагах 5.4–5.7.
