...
#### Шаг 3. Значения как переменные в терминале
...
#### Шаг 5. Подстановка значений в конфиги
...
#### Шаг 6. Устройства и ссылки (RU-сервер)
...
### 1.2. Сайт-заглушка и сертификат
...
### 2.2. Заглушки и сертификаты для масок
...
```json
{
  ...
  "routing": {
    "rules": [
      ...
      { "type": "field", "ip": ["geoip:private"], "outboundTag": "block" },
      { "type": "field", "protocol": ["bittorrent"], "outboundTag": "block" },
      { "type": "field", "domain": ["domain:ru", "domain:su", "domain:xn--p1ai"], "outboundTag": "direct" },
...
- Сайты в зонах `.ru`, `.su`, `.рф` и российские IP идут напрямую. Если какой-то нужный вам сайт в зоне `.ru` заблокирован, добавьте первым правилом `{ "type": "field", "domain": ["domain:этот-сайт.ru"], "outboundTag": "to-eu" }`.
...
### 6.4. Автообновления безопасности
