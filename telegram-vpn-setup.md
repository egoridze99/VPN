...
Выполняется на обоих серверах: сначала закрываем доступ, потом ставим пакеты. Файла значений ещё нет, поэтому `SSH_PORT` и `IP_СЕРВЕРА` на этом этапе вписывайте руками. В итоге наружу будут открыты только SSH на нестандартном порту, 80 и 443, а вход — только по ключу.
